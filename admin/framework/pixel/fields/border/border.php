...
class PF_Field_border extends PF_classFields {
	...
	public function render() {
		...
		if( ! empty( $args['all'] ) ) {
			...
			echo '<div class="pf--left pf--input">';
			...
			echo ( ! empty( $args['unit'] ) ) ? '<span class="pf--label pf--label-unit">'. $args['unit'] .'</span>' : '';
			echo '</div>';
		...
		if( ! empty( $args['color'] ) ) {
			...
			echo '<input type="text" name="'. $this->field_name('[color]') .'" value="'. $value['color'] .'" class="pf-color" />';
			echo '</div>';
		}
