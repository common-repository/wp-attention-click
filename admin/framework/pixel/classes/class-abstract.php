 ...
 *
 * @since 	1.0 		2019-02-26 16:22:22 	Release
 * @since 	1.2.2 		2019-03-20 14:04:43 	Removed spaces in the file header
 * @since	1.4.3		2019-05-10 11:24:36		Remove urlencode for fonts implode
 */
abstract class PF_classAbstract {
    ...
    public function add_enqueue_google_fonts() {
		...
		if( ! empty( $this->pre_fields ) ) {
			...
			foreach( $this->pre_fields as $field ) {
				...
				if( $field_type && $field_id ) {
					...
					PF::maybe_include_field( $field_type );

					$class_name = 'PF_Field_' . $field_type;
		...
		if( ! empty( $this->typographies ) && empty( $this->args['async_webfont'] ) ) {
			...
			$handle = 'pf-google-web-fonts-'. $this->unique;
    ...
    public function add_output_css() {
		...
		$this->output_css = apply_filters( "pf_{$this->unique}_output_css", $this->output_css, $this );
