...
     * start working correctly.
    ...
    public static function load_files_core(){
        ...
        // ─── Load class ────────
        $files[] = ['classes/class-abstract','classes/class-fields','classes/class-widget', 'classes/class-customize-options'];
        if( pf_is_user_login_allow() ){
            $files[] = ['classes/class-options','classes/class-pages','classes/class-metabox'];
        }
        // Fetch
        foreach($files as $file){
            foreach($file as $value){
     ...
     *
     * @since	1.0	2019-02-19 23:37:50 Release
     * @return	void
     */
     ...
     *
     * @since	1.0	2019-02-20 08:18:38 Release
     * @return	void
     */
                    ...
                    if ( ! empty( $values['options'] ) ) {
                        foreach( $values['options'] as $key => $settings ) {
                            if( ! empty( $settings ) ) {
                                ...
                                // HACK: here the customizer configuration is missing
                            }
                        }
                    }
                    // For Pages
...
         * Perform several validations to see if the file is:
     ...
     * Make a field wrap
     *
     * @since   1.3.5   2019-04-16 12:38:24     Add documentation of the function
     * @return  string|html
     */
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
        ...
        if( ! empty( $field_type ) ) {
            ...
            if( ! empty( $field['is_custom'] ) ){
                // FIXME: Check because now it does not send the custom fields here
                if( isset(self::$args_instanced[$unique]) ){
                    $path_custom = self::$args_instanced[$unique][$where]['args']['custom_fields_path'];
