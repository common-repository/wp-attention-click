 ...
 *
 * @since   1.5   2019-07-14 05:01:03   Release
 *
 */
if( ! class_exists( 'PF_Customize_Options' ) ) {
	class PF_Customize_Options extends PF_classAbstract {
		...
		public function add_customize_save( $wp_customize ) {
			do_action( "pf_{$this->unique}_save_before", $this->get_options(), $this, $wp_customize );
			do_action( "pf_{$this->unique}_saved", $this->get_options(), $this, $wp_customize );
			do_action( "pf_{$this->unique}_save_after", $this->get_options(), $this, $wp_customize );
		}
		...
		public function add_customize_options( $wp_customize ) {
			...
			if( ! class_exists( 'WP_Customize_Panel_PF' ) ) {
				PF::include_plugin_file( 'functions/customize.php'  );
			}
			...
			if( ! empty( $this->sections ) ) {
				...
				foreach( $sections as $section ) {
					...
					if( ! empty( $section['sub'] ) ) {
						...
						$wp_customize->add_panel( new WP_Customize_Panel_PF( $wp_customize, $panel_id, array(
		...
		public function add_section( $wp_customize, $section_id, $section_args, $panel_id ) {
			...
			if( ! empty( $section_args['assign'] ) ) {
				...
				$wp_customize->add_section( new WP_Customize_Section_PF( $wp_customize, $section_id, array(
			...
			if( ! empty( $section_args['fields'] ) ) {
				...
				$wp_customize->add_control( new WP_Customize_Control_PF( $wp_customize, $setting_id,
